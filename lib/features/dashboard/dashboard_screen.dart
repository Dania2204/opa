...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _load() async {
    ...
    setState(() {
      ...
      _delivered = orders.where((o) => o.status == DeliveryStatus.delivered).length;
  ...
  Widget build(BuildContext context) {
    final actions = <_QA>[
      if (user.role.canCreateOrders)
        _QA(Icons.add_road_rounded, 'New Order', PaeColors.secondary, () {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Go to Map tab to manage deliveries')));
        }),
      if (user.role.canSubmitReport)
        _QA(Icons.assignment_add, 'New Report', PaeColors.success, () {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Go to Reports tab')));
        }),
      if (user.role.canAddSchoolLocation)
        _QA(Icons.add_location_alt_rounded, 'Add School Pin', PaeColors.info, () {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Go to Map tab to add school location')));
        }),
      if (user.role.canManageUsers)
        _QA(Icons.person_add_rounded, 'Add Personnel', PaeColors.primaryLight, () {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Go to Personnel tab')));
        }),
    ];
...
class _OrderTile extends StatelessWidget {
  ...
  Color get _statusColor {
    switch (order.status) {
      case DeliveryStatus.pending:   return PaeColors.statusPending;
      case DeliveryStatus.assigned:  return PaeColors.statusAssigned;
      case DeliveryStatus.enRoute:   return PaeColors.statusEnRoute;
      case DeliveryStatus.delivered: return PaeColors.statusDelivered;
    }
  }
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.local_shipping_rounded,
                color: PaeColors.primary, size: 22),
          ),
