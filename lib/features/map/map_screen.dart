...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> _saveSchool() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('School location saved')),
    );
  }
  ...
  Future<void> _createOrder(SchoolLocation school) async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Order created for ${school.name}')),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final activeOrders =
        _orders.where((o) => o.status != DeliveryStatus.delivered).length;
        ...
        children: [
          ...
          Positioned(
            ...
            child: GradientHeader(
              title: S.mapTitle,
              subtitle: '${_schools.length} schools / $activeOrders active orders',
              actions: [
                IconButton(
        ...
        children: [
          Row(
            ...
            children: [
              ...
              IconButton(
                icon: const Icon(Icons.close, color: PaeColors.textSecondary),
                onPressed: onCancel,
              ),
            ],
          ),
...
class _DriverOrderPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final myOrder = orders
        ...
        .firstOrNull;
    final available =
        orders.where((o) => o.isOpen && o.status == DeliveryStatus.pending).toList();
